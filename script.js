...
const mallaData = {
    ...
    asignaturas: [
        ...
        {id: "HIPA121", nombre: "Patología molecular", anio: 3, semestre: 1, prerrequisitos: ["ESME125"]},
        {
            ...
            semestre: 1, 
            prerrequisitos: [
                "DYRE060", "CIDI080", "ESME001", "ESME120", 
                "ESME115", "ESME125", "ESME140", "PEDI130", 
                "SALP132", "ESME240", "PSIQ134", "SALP142"
            ]
        },
        ...
        {
            id: "CCOS290", 
            nombre: "Pregrado de medicina interna", 
            anio: 7, 
            semestre: 1, 
            prerrequisitos: {
                tipo: "todos_semestres",
...
// Función para obtener el número de semestre global
function getSemestreGlobal(anio, semestre) {
    return (anio - 1) * 2 + semestre;
}
...
function cumplePrerequisitos(ramo) {
    ...
    if (Array.isArray(ramo.prerrequisitos)) {
        // Prerrequisitos normales (lista de IDs)
        return ramo.prerrequisitos.every(id => aprobados[id] === true);
    } 
    else if (ramo.prerrequisitos.tipo === "todos_semestres") {
        // Prerrequisitos masivos
        const { desde, hasta, excepto = [] } = ramo.prerrequisitos;
        
        // Obtener todos los ramos en el rango de semestres
        const ramosRequeridos = mallaData.asignaturas.filter(a => {
            const semestreGlobal = getSemestreGlobal(a.anio, a.semestre);
            return semestreGlobal >= desde && 
                   semestreGlobal <= hasta &&
                   !excepto.includes(a.id);
        });
        
        // Verificar que todos estén aprobados
        return ramosRequeridos.every(r => aprobados[r.id] === true);
    }
    return true;
}

// Función para renderizar la malla (igual que antes)
function renderMalla() {
    // ... (código sin cambios del renderMalla anterior)
}
